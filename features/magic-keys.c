...
void magic_execute_key(
    ...
    // wants to store it.
    if (store_as_keytype >= NEXT_KEY && store_as_keytype < MAGIC_KEYS_COUNT) {
...
void magic_tap_key(magic_key_t keytype) {
    magic_execute_key(magic_keys[keytype].function_pointer, magic_keys[keytype].keycodes, keytype);
